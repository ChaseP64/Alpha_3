...
class ProjectController(QObject):
    ...
    def __init__(self, main_window: "MainWindow"):
        ...
        # self._serializer = ProjectSerializer() # Moved to ProjectFileController
    ...
    def on_new_project(self):
        """Handles the 'New Project' action by delegating to the file controller."""
        self.main_window.file_controller.on_new_project()

    def on_open_project(self):
        """Handles the 'Open Project' action by delegating to the file controller."""
        self.main_window.file_controller.on_open_project()

    def on_save_project(self, save_as=False) -> bool:
        """Handles the 'Save Project' action by delegating to the file controller."""
        return self.main_window.file_controller.on_save_project(save_as)
...
Do you want to save them?""",
    ...
    def rebuild_surfaces_for_layers(self, layer_names: set[str]):
        """Rebuilds surfaces derived from a specific set of layers."""
        from digcalc_project.src.core.geometry.surface_builder import (
            SurfaceBuilder,
            SurfaceBuilderError,
        )
        ...
        for surf_name, surf in list(project.surfaces.items()):
            src_layer = getattr(surf, "source_layer_name", None)
            if not src_layer or src_layer not in layer_names:
                continue
        ...
        if rebuilt_count > 0:
            self.logger.info("Rebuilt %d surface(s) for layers: %s", rebuilt_count, layer_names)
            self._rebuild_lowest()
            self.set_project_modified(True)
            self.surfaces_rebuilt.emit()
