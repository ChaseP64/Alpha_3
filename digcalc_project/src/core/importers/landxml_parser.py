...
class LandXMLParser(FileParser):
    ...
    def create_surface(self, name: str) -> Optional[Surface]:
        ...
        try:
            ...
            # Add triangles if available
            for triangle in self._triangles:
                surface.add_triangle(triangle)
            
            # If no triangles were parsed, generate them
            if not self._triangles and len(self._points) >= 3:
                tin_generator = TINGenerator()
                surface = tin_generator.generate_from_points(self._points, name)
            
            self.logger.info(f"Created surface '{name}' from LandXML data")
            return surface
            
        except Exception as e:
            self.log_error("Error creating surface from LandXML data", e)
            return None
    ...
    def _parse_surface_faces(self, surface_elem: ET.Element) -> None:
        ...
        for face_elem in enumerate(face_elements):
            try:
                ...
                # Get points by ID
                points = [self._point_map.get(vid) for vid in vertex_ids[:3]]
                if None in points:
                    continue
