...
class SurfaceBuilder:
    ...
    def build_from_polylines(
        ...
        """
        logger.info(
            f"Attempting to build surface from layer '{layer_name}' ({len(polylines_data)} polylines)."
        )
        ...
        for i, poly_dict in enumerate(polylines_data):
            try:
                ...
                logger.warning(
                    f"Skipping polyline {i} in layer '{layer_name}' due to data error: {e}",
                    exc_info=True,
                )
                continue
        ...
        logger.info(
            f"Extracted {num_unique_pts} unique 3D points with elevation from layer '{layer_name}'."
        )
        ...
        try:
            ...
            logger.error(
                f"Delaunay triangulation failed for layer '{layer_name}': {qe}", exc_info=True
            )
            raise SurfaceBuilderError(
            ...
            raise SurfaceBuilderError(
                f"An unexpected error occurred during triangulation: {e}"
            ) from e
        ...
        point_id_list = [
            p.id for p in surface_points_dict.values()
        ]  # Assumes order is preserved from enumerate
        ...
        for i, face_indices in enumerate(faces_np):
            try:
                ...
                logger.error(
                    f"Index error creating triangle {i} from face {face_indices}: {e_idx}. Point ID list length: {len(point_id_list)}"
                )
                continue  # Skip this triangle
            except KeyError as e_key:
                logger.error(
                    f"Key error creating triangle {i} (likely missing point ID): {e_key}. Face indices: {face_indices}"
                )
                continue  # Skip this triangle
        ...
        logger.info(
            f"Successfully built surface '{surface.name}' from layer '{layer_name}' (Rev: {revision})."
        )
        return surface
...
def lowest_surface(design: Surface, existing: Surface) -> Surface:
    ...
    *existing*.
    ...
    def _sorted(surface: Surface):
        return sorted(
            [(p.x, p.y, p.z) for p in surface.points.values()],
