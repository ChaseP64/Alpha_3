...
class TINGenerator:
    ...
    def generate_from_points(self, points: List[Point3D], name: str) -> Surface:
        ...
        if len(unique_points_list) < 3:
            self.logger.warning(
                f"Cannot generate TIN for '{name}': requires at least 3 unique points, found {len(unique_points_list)}."
            )
        ...
        self.logger.info(
            f"Generated TIN surface '{name}' with {len(surface.points)} points and {len(surface.triangles)} triangles."
        )
        return surface
...
def generate_tin(
    points: Union[np.ndarray, Sequence[Sequence[float]]], name: str = "TIN Surface"
) -> Surface:
