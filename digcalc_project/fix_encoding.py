    ...
    try:
        ...
        if b'\x00' in content:
            ...
            try:
                ...
                print(f"  ✓ File is now valid UTF-8")
                return True
            except UnicodeDecodeError:
                print(f"  ⚠ Could not decode as UTF-8, attempting UTF-16...")
                try:
                    # Try UTF-16 decoding
                    text = cleaned_content.decode('utf-16', errors='replace')
                    file_path.write_text(text, encoding='utf-8')
                    print(f"  ✓ Converted from UTF-16 to UTF-8")
                    return True
                except Exception:
                    print(f"  ⚠ UTF-16 failed, attempting direct binary cleanup...")
                    ...
                    print(f"  ✓ Sanitized binary content")
                    return True
        else:
            ...
            try:
                ...
                try:
                    ...
                    print(f"  ✓ Replaced invalid characters")
                    return True
    ...
    try:
        ...
        try:
            with codecs.open(str(temp_path), 'r', encoding='utf-8') as f:
                temp_content = f.read()
            print(f"  ✓ Temporary file is valid UTF-8")
            ...
            print(f"  ✗ Failed to verify temp file: {str(e)}")
            # Clean up
            if temp_path.exists():
                temp_path.unlink()
            return False
    except Exception as e:
        print(f"Error recreating {file_path}: {str(e)}")
        return False
    ...
    for i, file_path in enumerate(files, 1):
        ...
        if fix_file_encoding(file_path):
            ...
            try:
                ...
                print(f"  ⚠ File still has encoding issues, attempting full recreation")
