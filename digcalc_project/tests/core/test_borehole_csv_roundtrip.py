...
from digcalc_project.src.models.strata_models import (
    StrataStack, Material, BoreholeLog, LayerDepth,
)
from digcalc_project.src.services.borehole_csv_io import save_csv, load_csv

def _build_stack() -> StrataStack:
    st = StrataStack(id=1)
    clay = Material(id=1, name="Clay", colour="#964B00")
    st.materials.append(clay)
    bh = BoreholeLog(id=1, x=0.0, y=0.0,
                     layers=[LayerDepth(material_id=1, top_z=0.0, bottom_z=-5.0)])
...
def test_layer_order(tmp_path):
    ...
    bh = BoreholeLog(id=1, x=1.0, y=1.0,
                     layers=[
